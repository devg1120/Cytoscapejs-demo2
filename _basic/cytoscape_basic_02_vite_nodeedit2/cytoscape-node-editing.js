var stageId = 0;
export default function register(cytoscape, Konva) {
  if (!cytoscape || !Konva) {
    return;
  }
  cytoscape("core", "nodeEditing", init_cytoscape);
}

function init_cytoscape(opts) {
  var cy = this;
  ...
  var eUnselectNode,
    ...
    eFreeNode,
    eUndoRedo;
  var options = extend(true, defaults_setting(), opts);

  //var container = document.querySelector(cy.container());
  var container = cy.container();
  //var container = document.querySelector("#cy");
  ...
  if (container.querySelectorAll("#" + canvasElementId).length < 1) {
  ...
  if (Konva.stages.length < stageId) {
    stage = new Konva.Stage({
      ...
      //GS width: $container.width(),
      //GS height: $container.height()
  ...
  var sizeCanvas = db.debounce(function () {
    /* GS
                $canvasElement
                ...
                */
    canvasElement.setAttribute("height", container.height);
...
*/
    ...
    setTimeout(function () {
      ...
      canvasElement.style.left =
        canvasElement.offsetLeft - container.offsetLeft;
      canvas.getStage().setWidth(container.offsetWidth);
      canvas.getStage().setHeight(container.offsetHeight);
    }, 0);
  }, 250);

  sizeCanvas();

  //GS $(window).on('resize', sizeCanvas);
  ...
  //function Grapple (node, resizeControls, location, isActive) {
  class Grapple {
    constructor(node, resizeControls, location, isActive) {
      this.parent = node;
    ...
    bindActiveEvents() {
      ...
      var eMouseDown = function (event) {
        ...
        setWidthFcn = node.isParent()
          ? options.setCompoundMinWidth
          : options.setWidth;
        setHeightFcn = node.isParent()
          ? options.setCompoundMinHeight
          : options.setHeight;

        getWidthFcn = function (node) {
          if (node.isParent()) {
            return Math.max(
              parseFloat(options.getCompoundMinWidth(node)),
              childrenBBox.w,
            );
          }
        ...
        getHeightFcn = function (node) {
          if (node.isParent()) {
            return Math.max(
              parseFloat(options.getCompoundMinHeight(node)),
              childrenBBox.h,
            );
          }
        ...
        event.target.getStage().container().style.cursor =
          options.cursors[translateLocation[self.location]];
        ...
        canvas.getStage().on("contentMouseup", eMouseUp);
        //canvas.getStage().on("contentMousemove", eMouseMove);
      };
      ...
      var eMouseUp = function (event) {
        cy.style()
          ...
          .update();
        self.shape.getStage().container().style.cursor =
          options.cursors.default;
        cy.boxSelectionEnabled(true);
        cy.panningEnabled(true);
        setTimeout(function () {
      ...
      var eMouseMove = function (event) {
        ...
        cy.batch(function () {
          ...
          if (
            (isAspectedMode && location.indexOf("center") >= 0) ||
            options.isNoResizeMode(node)
          )
            return;
          ...
          if (isAspectedMode) {
            ...
            var isCrossCorners =
              location == "topright" || location == "bottomleft";
            if (xWidth > xHeight)
      ...
      var eMouseEnter = function (event) {
        //console.log("eMouseEnter");
        event.target.getStage().container().style.cursor =
          options.cursors[translateLocation[self.location]];
      };

      var eMouseLeave = function (event) {
        //console.log("eMouseLeave");
        if (event.target.getStage() != undefined)
          event.target.getStage().container().style.cursor =
            options.cursors.default;
      };
    ...
    updateShapePosition(startPos, width, height, gs) {
  ...
  function ResizeCue(node, resizeControls) {
    this.parent = node;
    this.resizeControls = resizeControls;
  ...
  ResizeCue.prototype.updateShapePosition = function (
    startPos,
    width,
    height,
    cw,
  ...
  function isResizedToContent(node) {
    ...
    return (
      width === minWidth * resizeToContentSizeMultiplier &&
      height === minHeight * resizeToContentSizeMultiplier
    );
  }

  function getGrappleSize(node) {
    return (
      Math.max(1, cy.zoom()) *
      options.grappleSize *
      Math.min(node.width() / 25, node.height() / 25, 1)
    );
  }

  function getResizeCueHeight(node) {
    return (
      Math.max(1, cy.zoom()) *
      options.grappleSize *
      1.25 *
  ...
  function getResizeCueWidth(node) {
    return (
      Math.max(1, cy.zoom()) *
      options.grappleSize *
      1.25 *
  ...
  function defaultResizeToContent(params) {
    var self = params.self;
    var node = self.parent;

    var setWidthFcn = node.isParent()
      ? options.setCompoundMinWidth
      : options.setWidth;
    var setHeightFcn = node.isParent()
      ? options.setCompoundMinHeight
      : options.setHeight;
    ...
    if (params.firstTime) {
      ...
      params.oldWidth = node.isParent()
        ? options.getCompoundMinWidth(node)
        : node.width();
      params.oldHeight = node.isParent()
        ? options.getCompoundMinHeight(node)
        : node.height();
      if (node.isParent()) {
        params.oldBiasLeft = options.getCompoundMinWidthBiasLeft(node) || "50%";
        params.oldBiasRight =
          options.getCompoundMinWidthBiasRight(node) || "50%";
        params.oldBiasTop = options.getCompoundMinHeightBiasTop(node) || "50%";
        params.oldBiasBottom =
          options.getCompoundMinHeightBiasBottom(node) || "50%";
      }
...
/*
    (this.nativeMax = Math.max), (this.nativeNow = Date.now);
  ...
  debounce(func, wait, options) {
    ...
    function delayed() {
      var remaining = wait - (that.now() - stamp);
